...
type Acknowledgement = NodeCG.Acknowledgement;

function setAll<T>(prop: string, value: T, ack: Acknowledgement | undefined) {
...
nodecg.listenFor("clear-donations", (_value: unknown, ack?: Acknowledgement) => {
    setAll("read", true, ack);
});

nodecg.listenFor("approve-all-donations", (value: ModStatus, ack?: Acknowledgement) => {
    setAll("modStatus", value, ack);
});


function searchAndSet<T>(id: string, prop: string, value: T, ack: Acknowledgement | undefined): Donation | undefined {
...
nodecg.listenFor("set-donation-read", ([dono, readVal]: [Donation, boolean], ack?: Acknowledgement) => {
    const d = searchAndSet(dono.id, "read", readVal, ack);
    if (d && readVal && d.modStatus === UNDECIDED) d.modStatus = APPROVED;
});

nodecg.listenFor("set-donation-shown", ([dono, shownVal]: [Donation, boolean], ack?: Acknowledgement) => {
    searchAndSet(dono.id, "shown", shownVal, ack);
});

nodecg.listenFor("set-donation-modstatus", ([dono, statusVal]: [Donation, ModStatus], ack?: Acknowledgement) => {
